...
void OpenGLMesh::render(bool pickingPass) {
    ...
    if (m_host->meshType() == Mesh::Triangle)
        glDrawElements(GL_TRIANGLES, (GLsizei) m_host->indices().size(), GL_UNSIGNED_INT, 0);
    else if (m_host->meshType() == Mesh::Line)
        glDrawElements(GL_LINES, (GLsizei) m_host->indices().size(), GL_UNSIGNED_INT, 0);
    else
        glDrawElements(GL_POINTS, (GLsizei) m_host->indices().size(), GL_UNSIGNED_INT, 0);
