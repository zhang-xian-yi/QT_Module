...
QVector3D getIntersectionOfLinePlane(Line l, Plane p) {
    ...
    if (isnan(t) && Load3dModelNS::LMGlobalData::GetInstance().log_level >= LOG_LEVEL_WARNING)
...
QVector3D getClosestPointOfLines(Line l1, Line l2) {
    ...
    if (isnan(n) && Load3dModelNS::LMGlobalData::GetInstance().log_level >= LOG_LEVEL_WARNING)
        dout << "Warning: NaN detected";
...
Line screenPosToWorldRay(QVector2D cursorPos, QVector2D windowSize, QMatrix4x4 proj, QMatrix4x4 view) {
    ...
    st = inv * st; st /= st.w();
    ed = inv * ed; ed /= ed.w();
    if (isnan(st) && Load3dModelNS::LMGlobalData::GetInstance().log_level >= LOG_LEVEL_WARNING)
        dout << "Warning: NaN detected";
    if (isnan(ed) && Load3dModelNS::LMGlobalData::GetInstance().log_level >= LOG_LEVEL_WARNING)
        dout << "Warning: NaN detected";
    QVector3D dir = (ed - st).toVector3D();
    if (dir.isNull() && Load3dModelNS::LMGlobalData::GetInstance().log_level >= LOG_LEVEL_WARNING)
